...
#define EXAMPLE_PLUGIN_DEFINE(name, init_fn, task_fn, irq_fn) EXP_PLUGIN_DEFINE_SLOT_0(name, init_fn, task_fn, irq_fn)
#define EXAMPLE_PLUGIN_CODE EXP_PLUGIN_CODE(0)
#define EXAMPLE_PLUGIN_CONST EXP_PLUGIN_CONST(0)
#elif EXAMPLE_PLUGIN_SLOT == 1
#define EXAMPLE_PLUGIN_DEFINE(name, init_fn, task_fn, irq_fn) EXP_PLUGIN_DEFINE_SLOT_1(name, init_fn, task_fn, irq_fn)
#define EXAMPLE_PLUGIN_CODE EXP_PLUGIN_CODE(1)
#define EXAMPLE_PLUGIN_CONST EXP_PLUGIN_CONST(1)
#elif EXAMPLE_PLUGIN_SLOT == 2
#define EXAMPLE_PLUGIN_DEFINE(name, init_fn, task_fn, irq_fn) EXP_PLUGIN_DEFINE_SLOT_2(name, init_fn, task_fn, irq_fn)
#define EXAMPLE_PLUGIN_CODE EXP_PLUGIN_CODE(2)
#define EXAMPLE_PLUGIN_CONST EXP_PLUGIN_CONST(2)
#else
#define EXAMPLE_PLUGIN_DEFINE(name, init_fn, task_fn, irq_fn) EXP_PLUGIN_DEFINE_SLOT_3(name, init_fn, task_fn, irq_fn)
...
enum
{
	...
	EXAMPLE_UART_ACTIVITY_BIT = 0x02u,
...
EXAMPLE_PLUGIN_CODE static bool example_slot_init(exp_stack_t *stk, uint8_t expid)
{
	...
	if ((stk == 0) || (expid >= EXP_PLUGIN_SLOT_COUNT))
	{
		return false;
	}
